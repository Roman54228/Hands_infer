...
def predict_hand_gesture(image: np.ndarray, model: TrtRunner):
    ...
    img_resized = img_pil.resize((400,400), Image.BILINEAR)
...
def detect_hands_in_video():
    ...
    with vidu.PDstream(device, 1) as stream:
        ...
        if streamname == "ToF":
            ...
            stream.set("ToF::StreamFps", 30)
            stream.set("ToF::Exposure", 0.1)
            stream.set("ToF::Distance", 7.5)
            stream.set("ToF::DepthMedianBlur", 0)
            stream.set("ToF::DepthFlyingPixelRemoval", 1)
            stream.set("ToF::Threshold", 100)
            stream.set("ToF::Gain", 6)
            ...
            while True:
                ...
                if len(images) == 2:
                    ...
                    for i, box in enumerate(results[0].boxes.xyxy):
                        ...
                        inp_crop = cv2.flip(cv2.resize(cropped_resized, (640,640)), -1)
                        # cv2.imwrite('temp.png', inp_crop)
                        cls_class, conf = predict_hand_gesture(inp_crop, trt_model)
                        ...
                        # sender.send(address=f"/gestures/gesture_{i}", data=cls_class)
                        # Рисуем bounding box и ID
                        # color = (0, 255, 0)
                        track_id = 0
                        color = map_colors[cls_class]
                        cv2.rectangle(draw_image, (x1_smooth, y1_smooth), (x2_smooth, y2_smooth), color, 2)
                    ...
                    for j_i, box in enumerate(results[0].boxes.xyxy[:4]):
                        x1, y1, x2, y2 = map(int, box.tolist())
                        preds = np.expand_dims(kps_preds[j_i], 0)[:,:,:2] * 256
                        preds = preds[0]
                        h, w, _ = list_src_crops[j_i].shape
                        points = []
                        for p_id, (x_norm, y_norm) in enumerate(preds):
                            x_norm, y_norm = x_norm / CROP_SIZE, y_norm / CROP_SIZE
                            px = int(x_norm * w)
                        ...
                        if p8 and p4:
                            ...
                            if dist_48 < 10:
                                ...
                                pred = 2
                                color = (102,255,255)
                            print(f'DIIIST {dist_48}')
                        # cv2.rectangle(draw_image, (x1, y1), (x2, y2), color, 2)

                        sender.send(address=f"/gestures/gesture_{j_i}", data=pred)
